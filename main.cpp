...
void STDMETHODCALLTYPE hkDrawIndexedInstanced(ID3D12GraphicsCommandList* dCommandList, UINT IndexCountPerInstance, UINT InstanceCount, UINT StartIndexLocation, INT BaseVertexLocation, UINT StartInstanceLocation) {
	...
	//if (Strides == 67 && rootIndex == 6) //example
	{
	    ...
	    if (scanning && (frameCount++ % 2) == 0)
	    {
	        ...
	        if (inner16 > (CB_ALIGN - sizeof(DirectX::XMFLOAT4))) // past 240
	        {
	            inner16 = 0;
	            scanBase += CB_ALIGN;
	    ...
	    // Compute CBV base address (256-byte aligned)
	    const size_t writeOffset = scanBase + inner16;
		//size_t writeOffset = countnum * 16; //2. brute force this if models are still black

	    if (g_pCustomConstantBuffer && writeOffset + sizeof(DirectX::XMFLOAT4) <= g_constantBufferSize)
	    {
	        const D3D12_GPU_VIRTUAL_ADDRESS bufferAddress = (g_pCustomConstantBuffer->GetGPUVirtualAddress() + writeOffset) & ~0xFFull;
	
	        // Bind CBV to this block — no CPU writes needed
